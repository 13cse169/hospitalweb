	...
	class Patient extends MY_Controller {
		...
		public function emergency(){
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				else{ $code = 'P'.date('Ymd').$PID; $AID  = 'AD'.date('Ymd').$PID; }
				...
				if ($Head == 'Emergency Facility') {
					...
					for ($i = 0; $i < count($_POST['usg']); $i++) { 
						$myArray[] = array(
							'pid' => $PID, 
							'type' => 'U.S.G', 
							'amount' => $_POST['usg_fee'][$i] ,
							'name' => $this->my_library->GetParam('outdoor_facility', 'facilit_name', array('od_facilit_id' => $_POST['usg'][$i])) 
						);
					}
				}

				else if ($Head == 'X-Ray') {
					for ($i = 0; $i < count($_POST['xray']); $i++) { 
						$myArray[] = array(
							'pid' => $PID, 
							'type' => 'X-Ray', 
							'amount' => $_POST['xray_fee'][$i] ,
							'name' => $this->my_library->GetParam('outdoor_facility', 'facilit_name', array('od_facilit_id' => $_POST['xray'][$i])) 
						);
					}
				}
