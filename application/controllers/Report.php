	...
	*/
	class Report extends MY_Controller {
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		    	...
		    	if ($this->uri->segment(3)) {
		    		...
		    		if( ($UrlData->doc_type == 'Doctor') || ($UrlData->doc_type == 'Asst. Doctor') ){
		    			...
		    			$SQL = "SELECT * FROM patient_facility WHERE operative_surgeon = '$UrlData->doc_id' AND particular = 'OPERATION' AND CAST(created_at AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date' GROUP BY aid";
		    		}
		...
		public function details(){
			...
			$OtherFclt = array('Advance', 'FinalIPD', 'Emergency',  'AMBULANCE', 'E.C.G', 'ECHO', 'PATHOLOGY TEST', 'U.S.G', 'X-RAY');
			$UrlData = json_decode(base64_decode($this->uri->segment(3)));
			...
			if (in_array($UrlData->cat_id, $OtherFclt)) {
    			switch ($UrlData->cat_id) {
    				...
    				case 'Emergency':
    					$SQL = "SELECT * FROM h_payment WHERE pay_type = 'Emergency' AND CAST(pay_date AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date' ORDER BY pay_date";
    					break;
	    		...
	    		if (count($myData)) {
		    		foreach ($myData as $key => $value) {
		    			if ($UrlData->cat_id == 'Advance' || $UrlData->cat_id == 'FinalIPD'  || $UrlData->cat_id == 'Emergency') {
		    				$total += $value->pay_amt;
		...
		public function indoor(){
	    	...
	    	if ($this->uri->segment(3)) {
	    		...
	    		if ($UrlData->cat_id == 'All') {
	    			...
	    			$OT = $this->M_Hospital->RunQuery("SELECT SUM(amount) AS amount FROM patient_facility WHERE CAST(created_at AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date'");
		...
		public function emergency(){
	    	...
	    	if ($this->uri->segment(3)) {
	    		...
	    		if ($UrlData->code == 'Emergency Facility')
                  ...
                  if($UrlData->code == 'All'){
		    			$SQL2 = "SELECT * FROM other_facility WHERE patient_type = 'Emergency' AND CAST(created_at AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date'";

		    			$ResData2 = $this->M_Hospital->RunQuery($SQL2);
	    		...
	    		if (count($ResData2)) {
		    		...
		    		';
		    			...
		    			$SQL3 = "SELECT * FROM other_facility WHERE patient_type = 'Emergency' AND $type AND CAST(created_at AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date'";

		    			$ResData3 = $this->M_Hospital->RunQuery($SQL3);

	    		if (count($ResData3)) {

		    		foreach ($ResData3 as $key => $value) {
