	...
	class Report extends MY_Controller {
		...
		public function outdoor(){
	    	...
	    	if ($this->uri->segment(3)) {
	    		...
	    		//$Result = $this->M_Hospital->RunQuery("SELECT * FROM outdoor_treat WHERE category LIKE '%$UrlData->code%' AND CAST(app_date AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date'");
	    		
			if($UrlData->code == 'All'){
		    			$SQL = "SELECT * FROM outdoor_treat WHERE CAST(app_date AS DATE) BETWEEN '$UrlData->f_date' AND '$UrlData->t_date' ORDER BY category";
		    		} else {
			...
			if (count($ResData)) {
		    		foreach ($ResData as $key => $value) {
		    			...
		    			$Data['Row'] .= '
		    				<tr>
		    					...
		    					<td>'.$this->my_library->GetParam('outdoor_facility', 'facilit_name', array('od_facilit_id' => $particular[$index])).'</td>
		    					<td>'.$value->name.'</td>
		    		...
		    		$Data['Row'] .= '
	    				<tr class="bg-info text-white">
	    					<td>'.++$count.'.</td>
	    					<td></td><td></td>
	    					<td></td><td class="font-weight-bold">Total Amount</td>
	    					<td></td>
	    					<td>'.number_format((float)$Total, 2, '.', '').'</td>
	    				</tr>
		    		';
